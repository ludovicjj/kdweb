...
export default function checkIpEntered(event)
{
    ...
    if (pressed_key === "enter") {
        ...
        const ip_array = text.split(" | ");

        if (JSON.stringify(ip_array) !== '[""]') {
            ip_array.forEach((ip) => {
                if (!ipv4_and_ipv6_regex.test(ip)) {
                    alert(`L'adresse IP suivante : ${ip} n'est pas valide`);
                    throw new Error(`L'adresse IP suivante : ${ip} n'est pas valide`);
                }
            })
        }
